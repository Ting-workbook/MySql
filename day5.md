...
```
##### 虽然我只能读到另外一个事务提交的数据，但是还是会出现问题，就是读取同一个数据发现前后不一致。 #####
...
```
##### 这种现象就叫做 幻读！！ #####
##### 事务a 和 事务b 同时操作一张表，事务a 提交的数据，也不能被 事务b 读到，就可以造成幻读。 #####
...
-- 此时SQL语句被卡住了！！
-- 因为现在是串行化，当王尼玛那边在进行 user 表写入操作的时候，张全蛋这边就不能写入操作，要写入操作的话需要排队。--

-- 王尼玛 - 北京
  commit;
-- 此时张全蛋那边就可以操作了
```
##### 当 user 表被另外一个事务操作的时候，其他事务里面的写操作，是不可以进行的。#####
##### 进入排队状态（串行化状态），直到王尼玛那边事务结束之后，张全蛋的这个写入操作才会执行。 #####
##### 前提是在没有等待超时的情况下可以操作。 #####
